...
flowchart TD
    ...
    E --> G["Банки деталей:<br/>хуки, развития, концовки,<br/>приёмы, провалы"]
    E --> H["Закономерности<br/>и углы"]
    F --> H
    G --> H
    H --> I["Критик"]
    I --> J["ПАКЕТ сценаристу<br/>АНАЛИЗ-НЕДЕЛИ владельцу<br/>боли в json"]
```
...
Claude сам создаст четыре файла:

| Файл | Что в нём |
|---|---|
| `card.md` | всё о товаре: назначения, состав, цена, упаковка |
| `claims.md` | что нельзя обещать в этой категории |
| `pain-topics.md` | темы, по которым искать жалобы людей |
| `trend-keys.md` | ключевые слова для поиска роликов (на английском) |
...
| `боли-<товар>.json` | машинный файл для других агентов, руками не открывают |
| `refs\` | скачанные ролики, кадры, расшифровки речи |

### Накопительное — имущество товара

Папка `products\<товар>\banks\` — шесть файлов, которые растут
с каждым анализом:
