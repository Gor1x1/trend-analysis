...
Claude сам создаст пять файлов. **Руками с нуля не заполняется ничего** —
...
все статусы остаются гипотезами. Замеры кладутся в `metrics/zamery.csv`
и `sheets/publikacii.csv`; после них `hook-scorer` проставит хукам
настоящие статусы, а `combo-keeper` запишет выигравшие связки.
